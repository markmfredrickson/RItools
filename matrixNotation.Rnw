...
\begin{align}
  d(\mathbf{Z}, \mathbf{x}_{(j)}) &= \sum_{b=1}^B w_b \left\{  \mathbf{Z}_{b}'
                       (\mathbf{x}_{b(j)} \cdot \mathbf{w}_{b(j)})/(\bar{w}_{b(j)} n_{tb}) -
(\mathbf{1} -
                                    \mathbf{Z}_{b})'(\mathbf{x}_{b(j)}\cdot
                                    \mathbf{w}_{b(j)})/[\bar{w}_{b(j)}(n
                                    - n_{tb})] \right\} \nonumber  \\
  &= \sum_{b=1}^B w_b h_b^{-1} \bar{w}_{b(j)}^{-1} \mathbf{Z}_{b}'
    (\mathbf{x}_{b(j)} \cdot \mathbf{w}_{b(j)}) - \sum_{b=1}^B w_b
...
\begin{equation} \label{eq:2}
  d(\mathbf{Z}, \mathbf{x}) = \sum_{b=1}^B \frac{w_b}{h_b}
...
\item Expression \eqref{eq:2} somewhat obscures the important
  ...
  weighting suggested by H\&B 2008, one should have $w_{b} \propto
  \bar{w}_{b}h_{b}$.
...
\item By normalizing weights within block, the onus of accounting for
  differences in sample/population size across blocks is placed
  entirely on $\{w_{b}\} $.
