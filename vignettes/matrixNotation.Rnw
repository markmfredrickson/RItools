...
\vec{w}_{bi} \cdot \vec{x}_{bi} \right] \label{eq:13a} \\
  &= \sum_{b=1}^B \frac{s_b}{h_b} \sum_{i\leq n_{b}}{Z}_{bi}
\left[ \frac{{w}_{bi}}{\bar{w}_b} \cdot \vec{x}_{bi} - n_{b}^{-1} \sum_{i\leq n_{b}}
\frac{{w}_{bi}}{\bar{w}_b} \cdot \vec{x}_{bi} \right] , \label{eq:13b}
\end{align}
...
(\texttt{I don't think this weight construction fixes the issued noted above.}),
...
of $d()$ in \eqref{eq:13a}--\eqref{eq:13b} calls for stratum-wise alignment of $\mathbf{x} \cdot \mathbf{w}$, without further weighting.
