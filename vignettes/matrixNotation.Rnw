...
@ 
Note that \texttt{pv.c.wt} is doing two things here, imposing unit
...
\begin{enumerate}
\item Cluster size $m_{bi}$ is replaced by the more general $w_{bi}$,
  indicating the sum of the unit weights associating with non-NA
  elements (dis-aggregated units of observation) within the cluster
  $i$ of stratum $b$.
\item In addition, $w_{bi}$ is allowed to differ for different
  ...
  $n_{b}$-vectors $\mathbf{m}_{b}$ we have $m \times p$ matrices
  $\mathbf{w}_{b}$; also $\bar{w}_{b}$ is a $p$-vector.
