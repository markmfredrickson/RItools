...
Now we restate these definitions using a notation consistent with
notation to be used in this section. Here $\mathbf{x}$ will be a
matrix of cluster means, calculated with weighting for cluster sizes
$w$; the notation $s_{b}$ will be used for the weight associating with
stratum $b$. Temporarily using $\mathbf{t}_{b}$ for the matrix of cluster
totals, $[x_{rc} w_{r} : r\leq n_{b}, c\leq p]$, the definition of \citet{hansen:bowers:2008} becomes
\begin{align}
   d(\mathbf{Z}, \mathbf{t}) =& \sum_{b=1}^B s_b h_b^{-1} \bar{w}_b^{-1} \mathbf{Z}_{b}' \mathbf{t}_{b} - \sum_{b=1}^B s_b
  \bar{w}_b^{-1} (n_b - n_{tb})^{-1} \mathbf{1}'\mathbf{t}_{b}   \nonumber \\
  =& \sum_{b=1}^B \frac{s_b}{h_b\bar{w}_b} \mathbf{Z}_{b}'
 (\mathbf{t}_{b}  - \bar{t}_{b}\mathbf{1}_{b}), \, 
  \bar{t}_{b} = n_{b}^{-1}\sum_{i} t_{bi}, \nonumber \\
  =& \sum_{b=1}^B \frac{s_b}{h_b\bar{w}_b} \sum_{i\leq n_{b}}{Z}_{bi}
     \left[ {w}_{bi} \vec{x}_{bi} - n_{b}^{-1} \sum_{i\leq n_{b}}
     w_{bi} \vec{x}_{bi}
     \right] \label{eq:11}  \\
  =& \sum_{b=1}^B \frac{s_b}{h_b\bar{w}_b} \sum_{i\leq n_{b}}{Z}_{bi}
     \left[ {w}_{bi} \vec{x}_{bi} - \left(\frac{\sum_{i\leq n_{b}}
     w_{bi}}{n_{b}}\right) \left(\frac{\sum_{i\leq n_{b}}
     w_{bi} \vec{x}_{bi}}{\sum_{i\leq n_{b}}
     w_{bi}}\right)
     \right] \nonumber \\
  =& \sum_{b=1}^B \frac{s_b}{h_b\bar{w}_b} \sum_{i\leq n_{b}}{Z}_{bi}
     \left[ {w}_{bi} \vec{x}_{bi} - \bar{w}_{b}\left(\frac{\sum_{i\leq n_{b}}
     w_{bi} \vec{x}_{bi}}{\sum_{i\leq n_{b}}
     w_{bi}}\right)
     \right]  \nonumber \\
  =& \sum_{b=1}^B \frac{s_b}{h_b\bar{w}_b} \sum_{i\leq n_{b}}{Z}_{bi}
      {w}_{bi} \vec{\tilde{x}}_{bi}, \, \text{where}\, \vec{\tilde{x}}_{bi} =  \vec{x}_{bi} - \frac{\bar{w}_{b}}{w_{bi}}\left(\frac{\sum_{i\leq n_{b}}
     w_{bi} \vec{x}_{bi}}{\sum_{i\leq n_{b}}
     w_{bi}}\right). \label{eq:12}
\end{align}
\texttt{Note that \eqref{eq:12} wound up rather strangely.  I suspect \texttt{balanceTest()} development prior to the issue89 branch was implicitly premised on this working out to $\sum_{b=1}^B \frac{s_b}{h_b\bar{w}_b} \sum_{i\leq n_{b}}{Z}_{bi}
      {w}_{bi}\left[ \vec{x}_{bi} - \left(\frac{\sum_{i\leq n_{b}}
     w_{bi} \vec{x}_{bi}}{\sum_{i\leq n_{b}}
     w_{bi}}\right)\right]$, which it isn't.}
 
 
The additional wrinkle to handle missingness: $w_{bi}$ is allowed to
differ for different variables, so instead of $n_{b}$-vectors
$\mathbf{w}_{b}$ we have $m \times p$ matrices $\mathbf{w}_{b}$;
entries in this matrix tally sums of weights for units that are not
missing on that variable.  (If records have been aggregated to the
cluster level, these are sums of the unit weights associating with non-NA
elements [dis-aggregated units of observation] within the cluster.) Corresponding to variable $j \leq p$ denote
the $j$th column of $\mathbf{w}_{b}$ as $\mathbf{w}_{b(j)}$; also
...
unit-wise product of similarly dimensioned vectors or matrices $\mathbf{a}$
and $\textbf{b}$, the pre-issue89 extension of the adjusted mean difference to accommodate missingness was 
\begin{align}
...
\end{align}
which \texttt{was mistakenly taken to be equivalent to \eqref{eq:2} below but in fact} is equivalent to
\begin{equation*}
\sum_{b} \frac{s_b}{h_b\bar{w}_b} \frac{\bar{w}_b}{\bar{w}_{b(j)}} \mathbf{Z}_b'[(\mathbf{x}_{b(j)} \cdot \mathbf{w}_{b(j)}) - n_b^{-1} \sum_{i\leq n_b}x_{bi(j)}{w}_{bi(j)}]. 
\end{equation*}
Alternatively, perhaps it was a mistake to use per-variable weights in the
denominators of~\eqref{eq:0} and~\eqref{eq:1}.  We might simply have
replaced those $\bar{w}_{b(j)}$s with $\bar{w}_{b}$s.  Observing
the convention that the first column of $\mathbf{w}$ records sums of
weights associating with each non-null element record in the cluster,
write $\bar{w}_b = n_b^{-1} \sum_{i\leq n_b} w_{bi(1)}$.  (Thus
$w_{bi(j)} \leq w_{bi(1)}$, all $i, j$.)  The alternate definitional
extension would be
\begin{align}
d(\mathbf{Z}, \mathbf{x}_{(j)}) &=
\sum_{b=1}^B s_b \left\{ \mathbf{Z}_{b}' (\mathbf{x}_{b(j)} \cdot
\mathbf{w}_{b(j)})/(\bar{w}_{b} n_{tb}) - (\mathbf{1} -
\mathbf{Z}_{b})'(\mathbf{x}_{b(j)}\cdot
\mathbf{w}_{b(j)})/[\bar{w}_{b}(n - n_{tb})] \right\} \nonumber \\
 d(\mathbf{Z}, \mathbf{x})&=
\sum_{b=1}^B \frac{s_b}{h_b\bar{w}_b} \sum_{i\leq n_{b}}{Z}_{bi}
\left[ \vec{w}_{bi} \cdot \vec{x}_{bi} - n_{b}^{-1} \sum_{i\leq n_{b}}
\vec{w}_{bi} \cdot \vec{x}_{bi} \right] , \label{eq:13}
\end{align}
in parallel with \eqref{eq:11}.  Either definition calls for figuring matrices of
weighted totals of observed $x$'s by cluster, $(\mathbf{w}_{b} \cdot
\mathbf{x}_{b})$, and aligning them around their simple cluster means.

In the pre-issue89 implementation, the first step was to \textit{align} the
$\mathbf{x}_{b}$ submatrices, ie to shift them in such a way that their
weighted means will be 0, yielding $\tilde{\mathbf{x}}_{b}$. This was
done in an attempt to simplify \eqref{eq:1}  by causing its
second term to vanish. (\texttt{Not so!  Since $\mathbf{Z}_{b}'\mathbf{w}_{b(j)}$ in general has nonzero variance, you can't just impose an additive shift on $x$ and be left with the same adjusted diff.}) 
Then we applied the $\bar{w}_{b}^{-1}$
stratum-wise weight factors, by multiplying them through by weight
matrices $\tilde{\mathbf{w}}_{b}$,
where $\tilde{\mathbf{w}}_{b(j)} = \mathbf{0} $ if
$\mathbf{w}_{b(j)}  = \mathbf{0} $, otherwise
$\tilde{\mathbf{w}}_{b(j)} = 
\bar{w}_{b(j)}^{-1} \mathbf{w}_{b(j)}$. 
\texttt{Previously I erroneously inferred} that  \eqref{eq:1} is then equivalent to 
\begin{align} 
  d(\mathbf{Z}, \mathbf{x})  &= 
                               \sum_{b=1}^B s_b h_b^{-1} \bar{w}_{b(j)}^{-1} \mathbf{Z}_{b}'
    [(\tilde{\mathbf{x}}_{b(j)} \cdot \tilde{\mathbf{w}}_{b(j)}) \bar{w}_{b(j)}]
  \nonumber \\
  &= \sum_{b=1}^B \frac{s_b}{h_b}
  \mathbf{Z}_{b}' (\tilde{\mathbf{x}}_{b} \cdot
  \tilde{\mathbf{w}}_{b}), \label{eq:2}
\end{align}
where $\cdot$ indicates unit-wise product. In contrast,
the alternate definition
of $d()$ in \eqref{eq:13} calls for stratum-wise alignment of
$\mathbf{x} \cdot \mathbf{w}$, with out further weighting. \texttt{In
  issue89 branch I remove the multiplying through by
  $\bar{w}_{b}^{-1}$, in the process reverting the meaning of
  ``stratum weight ratio'' from  $\frac{s_b}{h_b}$ to $\frac{s_b}{h_b\bar{w}_b}$, which aligns
  better w/ H\& B08 and is a more natural interpretation of that phrase. See c70cbe1.}
...
\item The default
  weighting suggested by H\&B 2008 is $s_{b} \propto
...
$\pi_{bi}=\mathbf{P}(Z_{bi} = 1)$.  To do this set stratum weights $s_{b}$ to be proportional to the sum of sample inclusion weights
for stratum $b$, $\left(\sum_{i} w_{bi} \right) =
n_{b}\bar{w}_{b} $, in order to align with a target population for
inference.  Then one has \texttt{fix up from here}
\begin{align*}
...
\subsection{Potential simplification of basic calcs}

Note that by \eqref{eq:13}, 
\begin{equation*}
 d(\mathbf{Z}, \mathbf{x})=
\sum_{b=1}^B \left( \mathbf{Z}_{b} - \frac{n_{tb}}{n_{b}} \right)'
\frac{s_b}{h_b\bar{w}_b} \mathbf{x}_{b} .
\end{equation*}
Similarly, from \citet{hansen:bowers:2008}, 
\begin{align*}
  \mathrm{Cov}\left\{   d(\mathbf{Z}, \mathbf{x}) \right\}  &=
\sum_{b=1}^{B} h_{b}\left( \frac{s_b}{h_b\bar{w}_b} \right)^{2}
 \frac{1}{n_{b}-1} \mathbf{x}_{b}'\mathbf{x}_{b} \\
  &= \sum_{b=1}^{B} \frac{h_{b}}{n_{b}-1} \left(
    \frac{s_b}{h_b\bar{w}_b} \mathbf{x}_{b} \right)' \left(
    \frac{s_b}{h_b\bar{w}_b} \mathbf{x}_{b} \right) .
\end{align*}

I think we could calculate this and the d's separately, then use
`base::solve()` for the omnibus statistic, implicitly relying on QR
rather than SVD.  
