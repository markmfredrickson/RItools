...
balanceTest(pr ~ date + t1 + t2 + cap + ne + ct + bw + cum.nF + strata(pt), data = testdata,impfn=mean.default)
...
balanceTest(pr ~ date + t1 + t2 + cap + ne + ct + bw + cum.nF + strata(pt), data = testdata,na.rm=FALSE,impfn=mean.default)

##This isn't right either.
balanceTest(pr ~ date + t1 + t2 + cap + ne + ct + bw + cum.nF + strata(pt), data = testdata,na.rm=TRUE,impfn=mean.default)
...
try(balanceTest(pr ~ date + strata(badStrat),
             data=testdata), FALSE)
